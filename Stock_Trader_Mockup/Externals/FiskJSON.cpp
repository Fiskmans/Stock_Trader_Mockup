...
namespace ImGui
{
	bool Json(const char* label, FiskJSON::Object& aObject)
	{
		using namespace std::string_literals;

		ImGui::PushID(&aObject);

		if (aObject.Is<FiskJSON::Leaf>())
		{
			ImGui::Text("%s [%s]: %s", label,aObject.PrettyType().c_str(), aObject.Serialize(false).c_str());
		...
		{
			if (ImGui::TreeNode((label + " ["s + aObject.PrettyType() + "]: ").c_str()))
			{
				int counter = 0;
				for (auto& val : aObject.Get<FiskJSON::Array>())
				{
					ImGui::Json(std::to_string(counter).c_str(), *val);
					counter++;
				}
				ImGui::TreePop();
			}
		}
		else if (aObject.Is<FiskJSON::Object>())
		{
			if (ImGui::TreeNode((label +  " ["s + aObject.PrettyType() + "]: ").c_str()))
			{
				for (auto& val : aObject)
				{
					ImGui::Json(val.first.c_str(), *(val.second));
				}
				ImGui::TreePop();
			}
		}
